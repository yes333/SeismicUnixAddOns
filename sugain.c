...
"       tmin=0.0        min. time to apply time-variant tpow scaling    ",
"       tmax=(header)   max. time to apply time-variant tpow scaling    ",
...
"	sugain may help.						",
"									",
"	Hard zeros are skipped when compute normalization factor	",
"									",
...
main(int argc, char **argv)
{
	...
	int agc;        /* agc flag                                     */
        ...
        float twmin, twmax;  /* time window to apply time-variant gain like tpow */
	...
	tmin = tr.delrt/1000.0;		   /* millisecs to secs */
        ...
        /* Get parameters */
        if (!getparfloat ("tmin" , &twmin))     twmin    = tmin;
        if (!getparfloat ("tmax" , &twmax))     twmax    = tmin + (nt - 1)*dt;
        else twmax = MIN(twmax, tmin + (nt - 1)*dt);
	...
	if (!panel) { /* trace by trace */
		...
		do {
			...
			if (!(tr.mark || mark) ) {
				gain(data, tpow, epow, gpow, agc, gagc, qbal,
				     ...
				     iwagc, tmin, dt, nt, maxbal ,pclip ,nclip, twmin, twmax );
		...
		gain(data, tpow, epow, gpow, agc, gagc, qbal,
		     pbal, mbal, scale, bias, trap, clip, qclip,
		     iwagc, tmin, dt, nt*ntr, maxbal ,pclip ,nclip, twmin, twmax );
...
void do_tpow(
	float *data,		/* the data			*/
        ...
        float twmin,            /* time window for application  */
        float twmax             /* time window for application  */
)
{
		...
		/* protect against negative tpow */
                if (tmin > twmin) twmin=tmin;
                n1 = NINT((twmin - tmin)/dt);
                n2 = NINT((twmax - tmin)/dt);
		tpowfac[0] = (twmin == 0.0) ? 1.0 : pow(twmin, tpow);
                for (i = 1; i < nt; ++i) {
                    if (i <= n1) tpowfac[i] = tpowfac[0];
                    else if  (i <= n2) tpowfac[i] = pow(tmin + i*dt, tpow);
                    else tpowfac[i] = tpowfac[n2];                    
                }
...
void gain(float *data, float tpow, float epow, float gpow,
	  ...
	  register float tmin, register float dt, int nt, int maxbal 
          ,float pclip ,float nclip, float twmin, float twmax )
{
    float f_two  = 2.0;
    float f_one  = 1.0;
    float f_half = 0.5;
	...
	if (tpow) {
		do_tpow(data, tpow, tmin, dt, nt, twmin, twmax);
	}
	if (epow) {
