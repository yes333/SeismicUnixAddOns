...
"   scale=100      value(key) = azimuth * scale                          ",
"   az=1           azimuth convention flag                               ",
"                   0:    0 - 180 deg, reciprocity assumed               ",
"                   1: -180 ~ 180 deg, points from receiver to source    ",
"                  -1: -180 ~ 180 deg, points from source to receiver    ",
"                                                                        ",
"   offset=1       =0: set or not set offset header field                ",
"                  positive for azimuth 0 ~ 180 and negative -180 ~ 0    ",
"   scaleo=scalco  if not set, use value of keyword scalco               ",
...
"   is assumed, so azimuths go from 0 to 179.9999 degrees.               ",
 ...
 *  or from the midpoint to the source. Azimuths go from -180 ~ 180 degrees
 *   
...
main(int argc, char **argv)
{
    ...
    double scaleo=0.0;      /* value of scale for offset*/
    ...
    if (!getpardouble("scale",&scale))      scale = 100.0;
    if (!getpardouble("scaleo",&scaleo)) {
        scaleo = 0.0;
    } else {
        warn("Input scaling factor for coordinates/offset scaleo=%5.3f", scaleo);
    }
    if (!getparint("az",&az))               az = 1;
    if (!getparint("offset",&oflag))        oflag = 1;
    ...
    while (gettr(&tr)) {
        ...
        azimuth = computeAzimuth(sx,sy,gx,gy,scale,az);
